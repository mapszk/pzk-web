...
const ContactForm: React.FC<Props> = ({theme}) => {
    ...
    const handleSubmit = (name:string, email:string, msg:string, endpoint:string) => {
        ...
        fetch(endpoint, {
            ...
            setAlertText({msg: "An error has ocurred, please try again", type: "error"})
            setAlert(true)
            setTimeout(():void => {
                setAlert(false)
                setAlertText({})
    ...
    return (
        <>
            ...
            .alert{
                ...
                background-color: ${alertText.type === 'error' ? "crimson" : colors.verde};
                display: flex;
                align-items: center;
            ...
            button{
                ...
                font-family: inherit;
                font-size: 1rem;
                transition: background-color .5s ease, border .2s ease, color .5s ease;
                cursor: pointer;
            } 
            button[disabled]{
                cursor: wait;
